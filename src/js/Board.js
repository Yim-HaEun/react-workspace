...
function Board(){
    ...
    const savedPosts = JSON.parse(localStorage.getItem('posts')) || []; //만약 저장된 게시글이 없으면 []빈 베열 가지고 옴
...
const addPost = () =>{
    ...
    //localStorage밑에 Item과 setItem 존재함
    //'posts' 키에 현재 게시글 목록과 새로운 게시글을 추가한 배열을 JSON 문자열로 변환해서 저장 
    localStorage.setItem('posts', JSON.stringify([...posts,newPost]));//입력된 게시글을 로컬 스토리지에 저장

   
} //게시글 삭제하기 
//todoList 할일목록 유지할 수 있도록 추가해주기
